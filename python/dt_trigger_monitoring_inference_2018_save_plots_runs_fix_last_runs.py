...
def plot_inst_lumi(x_val, y_val, z_val, title):
    fig, ax = plt.subplots()
    plt.xlabel("Time")
    plt.ylabel(r"Inst. Lumi. [$\times10^{30}$ cm$^2$s$^{-1}$]")
...
#for i in [+2]:
    ...
    #for j in [10]:
        ...
        #for k in [3]:
            ...
            title = "Fill/Run/Board: "+str(int_lumi2["fill"].iloc[0])+                            " / "+str("[321312, 319579, ") + str(run_to_process)+ "]"+" / " + board + "_MB" + str(k)
                
            #N.B.: The second run is the one used as reference
            (a, b, c) = plot_ratio_vs_ls(df_rates, [int(run_to_process), 319579, 321312], "lumi", station, board, 0,                            error, r"Inst. Lumi. [$\times10^{30}$ cm$^2$s$^{-1}$]", 
...
plot_vs_ls(df_rates_new[(df_rates_new["system"] == 2) & (df_rates_new["station"] == 4)], [int(run_to_process)],                "ls", "rate", "YB+1_S4", 0, "err", r"Inst. Lumi. [$\times10^{30}$ cm$^2$s$^{-1}$]",                "Rate [Hz]", title, "ro", False)
...
def plotFpVsLs(run, wheel, sector, station, title, df, algo, threshold, log, bound):
    ...
    plt.plot(df[(df["run"] == run)]["averageLS"],             df[(df["run"] == run)]["lumi"], "ro",             alpha=0.5, label="Inst. Lumi. $(x 10^{30} cm^2s^{-1})$")
...
plt.title("Distribution of scores: LOF (321312 + " + str(run_to_process) +")")
plt.legend(loc='best')
plt.ylabel('Frequency')
plt.xlabel('Score')
save_plot(plt)
